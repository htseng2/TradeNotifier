...
def plot_classification(df):
    ...
    # Plot sell signals (sell == 1) in red
    sell_signals = df[df["sell"] == 1]
    plt.scatter(sell_signals.index, sell_signals["Close"], color="red", label="Sell")

    # Plot predicted sell signals (predicted_sell == 1) in light coral
    predicted_sell_signals = df[df["predicted_sell"] == 1]
    plt.scatter(
...
def main():
    ...
    # model_path = f"models/lightgbm_model_20250209_122923.txt"
    model_path = f"live_models/lightgbm_model_sell_signal.txt"
